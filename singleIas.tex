...
Here however, we show that \replaced{ the evolution of intermediate-mass 
($\sim 1.8-2.5$\msun) 
helium stars -- a common product of binary interactions-- 
naturally leads to the formation of 
near-\mch, \one cores that}{near-\mch\ \one cores originating from intermediate mass helium stars ($\sim 1.8-2.5$\msun) ---a common product of binary interactions---} can ignite their residual carbon and oxygen explosively at  
...
consistent with classical \ias (Section\,\ref{sec:3}). 
 ...
 therefore may contribute significantly to the \ia  rate \replaced{at short 
 delay times in respect to star formation}{in young stellar populations} (Section\,\ref{sec:4}). 
...
after carbon burning without interruptions.\replaced{Upon experimentation, we found 
...
main result is not affected}{We further discuss this choice and its impact on the envelope evolution and the final mass in Section\,\ref{sec:5}}. 
...
40,000\,yr. During most of this time, the star \added{is a red giant with} a low-density convective envelope  
...
The last $\sim 5,000$\,yr of the evolution are characterized by rigorous burning in two neighbouring shells which eventually merge, resulting in a \iso{Ne}{20} flash.
...
employed in our calculations,  \added{\textsc{m1} briefly 
...
This \added{brief mass-loss phase} is perhaps not too unrealistic, as partial recombination of the carbon-rich  material would increase the porosity, leading to continuum-driven mass loss of similar magnitude \citep[e.g.][]{Owocki:2004zz} {\bf G\"{o}tz, not sure if this is indeed true, please check}. 
...
depleted and \deleted{are} nearing \mch. The ability of the core 
to grow in mass depends somewhat on the uncertain mass-loss rate during the final burning phases. If the envelope 
is lost too early during the SAGB phase \added{(which does not seem to be the case)}, then the two stars would 
...
adequately to \replaced{initiate carbon burning}{ignite carbon}. In turn, this triggers oxygen burning and a thermonuclear runaway at  $\logrhoc= 9.77$. This ignition density is much lower than the   $\logrhoc\ge 9.97$ typically expected for oxygen deflagrations  in pure NeO cores \citep{Jones:2018ule}. As a consequence, severe deleptonization due to \iso{Ne}{20} electron captures are avoided. 

\replaced{The evolution of \textsc{m2} is less complex, as the core}{\textsc{m2}} is composed mostly of carbon and oxygen, with $\abun{C}{12}=0.38$ and $\abun{O}{16}=0.60$ respectively. 
...
\cite{Chabrier:2004vw} IMF with $a=-2.35$ for $m_1 \ge 1$, and a $q-$distribution with $k=-0.1$ \deleted{as inferred for massive stars} \citep{Sana:2012px},
...
this  process will be  dynamically unstable, therefore leading to the removal of the envelope. If this process is indeed efficient, one would expect \added{at least half of the stars to lose their envelopes, hence, conservatively,} $f_{\rm int}\lesssim 0.5$ \citep{Sana:2012px}. 

Following the stripping of the envelope, any 
subsequent interaction should be sufficiently delayed, for the core to reach \mch. The post-CE orbital period distribution would generally favor \added{slightly} more compact configurations. Hence, only some fraction of the systems will be wide enough to allow 
...
overflow (RLO), could still leave behind stripped \one cores of sufficiently high mass. \added{For instance, the
...
their hydrogen  envelopes \deleted{to binary interactions}, are likely to explode 
as \ias instead of undergoing a core-collapse ECSN. The main reason for this seems to be the ignition of residual carbon following electron captures on \iso{Mg}{24}, which in turn leads to explosive oxygen burning at densities below $10^{9.77}$\,gr\,cm$^{-3}$ (Sections\,\ref{sec:2}). 
...
\added{Inspite of the mass-loss uncertainties, if viable, this progenitor channel opens up interesting possibilities for explaining some of the observed \ia diversity. Firstly, as either star in binary system could explode as \ia without accreting from its  companion, this channel could  produce events that resemple both SD and DD explosions. For the latter, in which the secondary explodes, the binary system would also have already survived a core-colapse SN from the more massive primary (Figure\,\ref{fig:rates}). 
The mass loss history would also influence the appearance of the explosion and the resulting SN remnant.                                                     }                                             
